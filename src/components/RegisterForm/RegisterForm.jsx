	...
	return (
		<div className='login-box'>
			...
			<input
				...
				className='login-box-input'
				value={props.dob}
				onChange={(ev) => props.setDob(ev.target.value)}
			/>
			<span
				style={{ color: '#999', fontWeight: 500, fontSize: 14, marginTop: 10 }}
			>
				(Last field is date of birth)
			</span>
			{props.fullName && props.address && props.dob && props.ssn ? (
				<span
					className='login-submit-button'
					onClick={() => props.setCurrentPage('userInformation')}
				>
					Next
				</span>
			) : (
				<span className='login-submit-button-disabled'>
					All fields required
				</span>
			)}
			<p className='sign-up-text' onClick={() => props.setCurrentView('login')}>
	...
	return (
		<div className='login-box'>
			...
			<input
				...
				onChange={(ev) => props.setPasswordVerify(ev.target.value)}
			/>
			{props.email &&
			props.password &&
			props.password.length > 7 &&
			props.passwordVerify === props.password ? (
				<span
					className='login-submit-button'
					onClick={() => props.setCurrentPage('summary')}
				>
					Next
				</span>
			) : (
			...
			<p
				...
				onClick={() => props.setCurrentPage('personalInformation')}
			>
				Go back
			</p>
		</div>
